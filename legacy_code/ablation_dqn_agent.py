...
class AblationDQNAgent:
    def __init__(self, env, num_time_bins=5, hidden_size=128, learning_rate=1e-3, gamma=0.99, 
                 use_action_mask=False, use_safety_mechanism=False, use_constraint_check=False):
        self.env = env
    ...
    def select_action(self, state, epsilon):
        # Safety mechanism check (only if enabled)
        if self.use_safety_mechanism:
            if state['remaining_time'][0] <= 0.2 * self.env.T_max and state['current_location'] != 0:
        ...
        if self.use_action_mask:
            ...
            # Standard DQN without action masking
            if random.random() < epsilon:
