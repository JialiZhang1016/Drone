...
# 读取xlsx文件并提取位置信息
df = pd.read_excel('config/realword_8/RL_8_locations.xlsx')
...
# 将距离矩阵存入config/config_8_real.json文件中
json_file = 'config/config_8_real.json'
...
for i, row in enumerate(distance_matrix):
    ...
    for j, d in enumerate(row):
        if i == j:
            ...
            new_row.append(round(d + random.randint(5, 20), 2))  # 加上随机增量
...
print("Updated config/realword_8/config_8_real.json successfully.")
